...
export default function PredictClient({ matches: _matches }: Props) {
  ...
  function selectGroupTeam(g: string, team: string) {
    ...
    // 1. ve 2. dolu → 3. sıraya tıklandı: best8 adayı
    setSelectedThirds(prev =>
      prev.includes(team)
        ? prev.filter(x => x !== team)
        : prev.length < 8 ? [...prev, team] : prev
    )
  }

  const allGroupsDone = Object.keys(GROUPS_DEF).every(g => {
    const sel = groups[g]
  ...
  function nextStep() {
    if (step === 0) setBest8(selectedThirds)
    setStep(s => s + 1)
  }
  ...
  function getNextPairs(prev: Record<string, string>, count: number): [string, string][] {
    const winners = Array.from({ length: count * 2 }, (_, i) => prev[`m${i}`] ?? '?')
  ...
  async function saveBracket() {
    ...
    setSaving(true)
    const bracket = { groups, best8, r32, r16, qf, sf, champion }
  ...
  const configs = [
    { label: 'Son 32 — 16 maçta kazananı seç', pairs: r32pairs, state: r32, setState: setR32 },
    { label: 'Son 16 — 8 maçta kazananı seç',  pairs: r16pairs, state: r16, setState: setR16 },
    { label: 'Çeyrek Final — 4 maç',            pairs: qfpairs,  state: qf,  setState: setQF  },
    { label: 'Yarı Final — 2 maç',              pairs: sfpairs,  state: sf,  setState: setSF  },
  ]
  ...
  return (
    <div className="min-h-screen pitch-stripes">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-0 mb-8 rounded-lg overflow-hidden border border-white/8">
          {STEPS.map((s, i) => (
            <button key={i} onClick={() => i < step ? setStep(i) : undefined}
              className={`flex-1 py-2 text-[10px] font-mono text-center transition-colors ${i === step ? 'bg-grass-500/20 text-grass-300' : i < step ? 'bg-white/5 text-white/50 cursor-pointer hover:bg-white/8' : 'bg-white/[0.02] text-white/20'}`}>
              {i < step ? '✓ ' : ''}{s}
            </button>
          ))}
        </div>
        ...
        {step >= 1 && step <= 4 && (
          <div>
            <p className="text-xs font-mono text-white/40 mb-4">{configs[step-1].label}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
              {configs[step-1].pairs.map(([t1, t2], i) => {
                const winner = configs[step-1].state[`m${i}`]
                return (
                  <div key={i} className="bg-white/[0.02] border border-white/8 rounded-xl overflow-hidden">
                    ...
                    {[t1, t2].map(t => (
                      <button key={t} onClick={() => configs[step-1].setState(prev => ({ ...prev, [`m${i}`]: t }))}
                        disabled={t === '?'}
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={nextStep}
                disabled={configs[step-1].pairs.some((_, i) => !configs[step-1].state[`m${i}`])}
          ...
          <div>
            ...
            {finpairs[0] && (
              <div className="bg-white/[0.02] border border-white/8 rounded-xl overflow-hidden mb-6 max-w-sm mx-auto">
                ...
                {finpairs[0].map(t => (
                  <button key={t} onClick={() => setChampion(t)} disabled={t === '?'}
                    ...
                    <span className="text-2xl">{gf(t)}</span>
            ...
            {champion && (
              <div className="text-center mb-6">
                <p className="text-4xl mb-2">{gf(champion)}</p>
