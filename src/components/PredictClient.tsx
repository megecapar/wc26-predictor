...
export default function PredictClient({ matches: _matches }: Props) {
  ...
  // 1. Sayfa açılınca localStorage'dan yükle
  useEffect(() => {
    try {
  ...
  // 2. Her değişimde localStorage'a yaz
  useEffect(() => {
    try {
      localStorage.setItem(DRAFT_KEY, JSON.stringify({ groups, selectedThirds, left, right, champion, step }))
    } catch {}
  ...
  const saveBracket = useCallback(async (silent = false) => {
    ...
    if (error) {
      if (!silent) setMsg('❌ ' + error.message)
    } else {
      setSaved(true)
      if (!silent) setMsg('✅ Bracket kaydedildi!')
        ...
        {step === 1 && (
          <div>
            ...
            <div className="flex flex-col gap-3 max-w-lg">
              {!userId ? (
                <Link href="/auth?returnTo=/predict"
                  className="block text-center py-3 bg-grass-500 text-white font-mono text-sm rounded-xl hover:bg-grass-400 transition-colors">
                  Kaydetmek için giriş yap — tahmin korunur
                </Link>
              ) : (
                <button onClick={() => saveBracket(false)} disabled={!champion || saving}
