...
import {
  getMatches, setMatches, getElo, setElo,
  getFinishedMatches, setFinishedMatches,
  setLastUpdate, FinishedMatch
} from '@/lib/blob'
import { fetchNosyOdds } from '@/lib/nosy'
...
async function fetchFinished(): Promise<FinishedMatch[]> {
  ...
  try {
    ...
    return (data.response ?? []).map((f: { teams:{home:{name:string},away:{name:string}}, goals:{home:number,away:number}, fixture:{date:string} }) => ({
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Nosy API — gerçek İddaa oranları
    const nosyOdds = await fetchNosyOdds(NOSY_API_KEY)
    log.push(`✓ ${Object.keys(nosyOdds).length} İddaa oranı alındı`)
    ...
    for (const match of matches) {
      ...
      // Nosy oranı varsa MS için kullan, yoksa Poisson
      ...
      match.overUnder = { ...match.overUnder,
        ...
        over:  { label:'2.5 Üst', value: no?.over25  ?? toOdd(pr.over25),  probability: rnd(pr.over25)  },
        under: { label:'2.5 Alt', value: no?.under25 ?? toOdd(pr.under25), probability: rnd(pr.under25) },
      }
      match.btts = {
